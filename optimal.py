#This is the best Training methode, and the best approche, that gave me optimal results that i found till now 
import pandas as pd
import matplotlib
import re
...
df = pd.read_csv('/content/drive/MyDrive/DataSet_Data_Science/malicious_phish.csv', nrows=100000)
...
#recursive  Feature selection methode with cross validation : 
#RFE , this is a process that removes features one by one , each time trainign the model , and evaluating it without the removed feature , this process is used to eleminate the least affecting features and chossing the more relevant ones
# Initialize RFECV
# the ML models that will be used to select the features is Random Forest
#step = 1 , that mean one feature will be removed on each iteration
#cv = 5 , cv stands for the number of folds to use in the cross validaton process, in this case 5 , 
#scoring : means the metrics that's used to assess the performance of the model in each iteration
...
rf_classifier.fit(X_train_rfecv, y_train)

# Predict on the test data
y_pred = rf_classifier.predict(X_test_rfecv)
...
for feature_list_index in rfecv.get_support(indices=True):
